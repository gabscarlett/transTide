function [a_m,a_pm] = bladeEM(U0,TSR,Nb,r,c,B,grid)
...
% input: ValuesTab       % Cl, Cd, alpha look-up table for each blade section
...
while (max(Err) > Ep)
  j=1+j;
                ...
                if size(grid.FCl.GridVectors,2) == 2
                    CL=grid.FCl(aoa, r(i));
                    CD=grid.FCd(aoa, r(i));
                else                   
                    CL=grid.FCl(aoa);
...
%               METHOD 2 - Glauret's empirical method
               ...
               R= @(PHI) sin(PHI)/(1-a) - cos(PHI)/((1+a_p)*TSr); % evaluate residual in terms of PHI only
              ...
              elseif Rpb(-0.25*pi)*Rpb(-Ep) < 0
                  ...
                  phi_0= zero (-0.25*pi,-Ep, eps, eps, Rpb);
                   ...
                   phi_0= zero (0.5*pi,pi-Ep, eps, eps, R);
               end
               ...
               % iterations
               if j > 200
                   warning(['BEM model failed to converge after ' mat2str(j) ' iterations'])
                   break                
               end
